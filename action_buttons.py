...
class ActionButtonsFrame(ttk.Frame):
    ...
    def _setup_frame(self):
        ...
        # Title label
        self.title_label = ttk.Label(self, text="Folgeaktionen:", 
        ...
        self.more_actions_btn = ttk.Button(self, text="Weitere Aktionen ▼",
                                         command=self._show_context_menu)
        self.more_actions_btn.pack(side=tk.RIGHT, padx=(5, 0))
        
        # History button
        self.history_btn = ttk.Button(self, text="Verlauf",
    ...
    def _refresh_buttons(self):
        ...
        # Create buttons for enabled actions
        enabled_actions = [a for a in self.available_actions if a.enabled]
        
        # Limit to first 4 main actions to avoid overcrowding
        main_actions = enabled_actions[:4]
        ...
        for i, action in enumerate(main_actions):
            ...
            # Add tooltip
            self._create_tooltip(btn, action.description)
        
        # Update "Weitere Aktionen" button visibility
        has_more_actions = len(enabled_actions) > 4
        if has_more_actions:
